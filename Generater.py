...
def generation(data, uid: str):
    ...
    if CharacterName in ["蛍", "空"]:
        CharacterImage = Image.open(
            f"{cwd}/character/{CharacterName}({element})/avatar.png"
        ).convert("RGBA")
    else:
        if CharacterCostume:
            CharacterImage = Image.open(
                f"{cwd}/character/{CharacterName}/{CharacterCostume}.png"
            ).convert("RGBA")
        else:
            CharacterImage = Image.open(
                f"{cwd}/character/{CharacterName}/avatar.png"
            ).convert("RGBA")
    ...
    if CharacterName == "アルハイゼン":
        CharacterAvatarMask2 = (
            Image.open(f"{cwd}/Assets/Alhaitham.png")
            .convert("L")
            .resize(CharacterImage.size)
        )
    else:
        CharacterAvatarMask2 = (
            Image.open(f"{cwd}/Assets/CharacterMask.png")
            .convert("L")
            .resize(CharacterImage.size)
        )
    ...
    # 武器
    Weapon = (
        Image.open(f"{cwd}/weapon/{WeaponName}.png").convert("RGBA").resize((128, 128))
    )
    ...
    WeaponRImage = Image.open(f"{cwd}/Assets/Rarelity/{WeaponRarelity}.png").convert(
        "RGBA"
    )
    ...
    for i, t in enumerate(["通常", "スキル", "爆発"]):
        ...
        Talent = (
            Image.open(f"{cwd}/character/{CharacterName}/{t}.png")
            .resize((50, 50))
            .convert("RGBA")
        )
    ...
    Clock = (
        Image.open(f"{cwd}/命の星座/{element}LOCK.png").resize((90, 90)).convert("RGBA")
    )
    ...
    for c in range(1, 7):
        if c > CharacterConstellations:
            ...
            CharaC = (
                Image.open(f"{cwd}/character/{CharacterName}/{c}.png")
                .convert("RGBA")
                .resize((45, 45))
            )
    ...
    D.text(
        (42, 397),
        f'Lv.{CharacterTalent["通常"]}',
        font=config_font(17),
        fill="aqua" if CharacterTalent["通常"] >= 10 else None,
    )
    D.text(
        (42, 502),
        f'Lv.{CharacterTalent["スキル"]}',
        font=config_font(17),
        fill="aqua" if CharacterTalent["スキル"] >= 10 else None,
    )
    D.text(
        (42, 607),
        f'Lv.{CharacterTalent["爆発"]}',
        font=config_font(17),
        fill="aqua" if CharacterTalent["爆発"] >= 10 else None,
    )
    ...
    for i, parts in enumerate(["flower", "wing", "clock", "cup", "crown"]):
        ...
        Preview = Image.open(f'{cwd}/Artifact/{details["type"]}/{parts}.png').resize(
            (256, 256)
        )
        ...
        Pmask = (
            Image.open(f"{cwd}/Assets/ArtifactMask.png")
            .convert("L")
            .resize(Preview.size)
        )
        ...
        MainIcon = (
            Image.open(f"{cwd}/emotes/{mainop}.png").convert("RGBA").resize((35, 35))
        )
        ...
        D.text(
            (374 + i * 373 - levlen, 749), f'+{details["Level"]}', font=config_font(21)
        )
